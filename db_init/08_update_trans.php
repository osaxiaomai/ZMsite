...
foreach ($products as $pid => $cat_id) {
    ...
    $main_index = $pid % count($image_ids);
    $main_img_id = $image_ids[$main_index];
    
    // The gallery gets the remaining 3 images
    $gallery_ids = $image_ids;
    unset($gallery_ids[$main_index]);
    $gallery_ids = array_values($gallery_ids); // re-index
    
    // 1. Set main thumbnail
    set_post_thumbnail($pid, $main_img_id);
    ...
    update_post_meta($pid, 'product_gallery', $gallery_ids);
    if (function_exists('update_field')) {
        update_field('product_gallery', $gallery_ids, $pid);
    }
}
